...
def run(digits, numbers):
    for roll in range(numbers):  # Repeats the amount of print outs, based on the amount of rolls
        printStr = ""
        for dice in range(digits):  # Prints the amount of numbers desired
            printStr = printStr + str(randint(0, 9))
        print(printStr)  # Prints the current roll's string


if __name__ == "__main__":
    digits = int(input("How Many digits? "))  # Gets the amount of Dice
    numbers = int(input("How many total numbers? "))  # Gets the amount of Rolls
    run(digits, numbers)
