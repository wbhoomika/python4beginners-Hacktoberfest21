...
def call_convert(rlabel1, rlabe12, inputn):
    ...
    if tempVal == "Celsius":
        ...
        k = round(float((float(tem) + 273.15)), 3)
        rlabel1.config(text="%.3f Fahrenheit" % f)
        rlabe12.config(text="%.3f Kelvin" % k)
    if tempVal == "Fahrenheit":
        c = round(float((float(tem) - 32) * 5 / 9), 3)
        k = c + 273
        rlabel1.config(text="%.3f Celsius" % c)
        rlabe12.config(text="%.3f Kelvin" % k)
    if tempVal == "Kelvin":
        c = round(float((float(tem) - 273.15)), 3)
        f = round(float((float(tem) - 273.15) * 1.8000 + 32.00), 3)
        rlabel1.config(text="%.3f Celsius" % c)
        rlabe12.config(text="%.3f Fahrenheit" % f)
    return
...
input_label = tk.Label(
    root, text="Enter temperature : ", background="#09A3BA", foreground="#FFFFFF"
)
input_entry = tk.Entry(
    root, textvariable=numberInput, borderwidth=5, fg="white", bg="#545454"
)
